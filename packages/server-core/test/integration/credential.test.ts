...
describe("credential endpoint", () => {
	...
	it("returns an error with a malformed bearer token type", async () => {
		const sub = "sub";
		...
		const response = await request(app)
			.post("/credential")
			.set("Authorization", `|earer ${access_token}`)
			.send({ credential_configuration_id, proofs: {} });
	...
	it("returns an error with an access token subject claim type invalid", async () => {
		const credential_configuration_id = "unknwown:configuration:id";
	...
	it("returns an error with an access token empty scope claim", async () => {
		const credential_configuration_id = "unknwown:configuration:id";
	...
	it("returns an error with an access token missing iat claim", async () => {
		const credential_configuration_id = "unknwown:configuration:id";
	...
	it("returns an error with an access token iat in the future", async () => {
		const credential_configuration_id = "unknwown:configuration:id";
	...
	describe("with a valid access token", () => {
		...
		it("returns an error with malformed dpop jwt signing algorithm name", async () => {
			const credential_configuration_id = "unknwown:configuration:id";
			...
			const dpop = `${header}.${payload}.signature`;
		...
		it("returns an error with invalid dpop jwt payload claim types", async () => {
			const credential_configuration_id = "unknwown:configuration:id";
			const { publicKey, privateKey } = await generateKeyPair("ES256");
			const claims = {
				jti: "jti",
		...
		it("returns an error with future dpop iat claim", async () => {
			const credential_configuration_id = "unknwown:configuration:id";
			const { publicKey, privateKey } = await generateKeyPair("ES256");
			const claims = {
				jti: "jti",
