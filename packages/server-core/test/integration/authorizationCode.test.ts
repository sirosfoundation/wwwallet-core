...
describe("authorization code - authorize", () => {
	...
	it("returns an error when request_uri iat is missing", async () => {
		const now = Date.now() / 1000;
		const secret = new TextEncoder().encode(protocols.config.secret);
		const client_id = "id";
		const request_token = await new EncryptJWT({
	...
	it("returns an error when request_uri client_id is missing", async () => {
		const now = Date.now() / 1000;
		const secret = new TextEncoder().encode(protocols.config.secret);
		const client_id = "id";
		const request_token = await new EncryptJWT({
...
describe("authorization code - token", () => {
	...
	it("returns an error when authorization code subject is missing", async () => {
		const grant_type = "authorization_code";
		const client_id = "id";
		const redirect_uri = "http://redirect.uri";
	...
	it("returns an error when authorization code iat is in the future", async () => {
		const grant_type = "authorization_code";
		const client_id = "id";
		const redirect_uri = "http://redirect.uri";
		const sub = "sub";
	...
	it("returns an error when authorization code scope claim type is invalid", async () => {
		const grant_type = "authorization_code";
		const client_id = "id";
		const redirect_uri = "http://redirect.uri";
		const sub = "sub";
