...
describe("deferred credential endpoint", () => {
	it("returns an error without body", async () => {
		const response = await request(app).post("/deferred-credential");
	...
	it("returns an error with an empty body", async () => {
		const response = await request(app).post("/deferred-credential").send({});
	...
	it("returns an error without transaction id", async () => {
		const response = await request(app).post("/deferred-credential").send({});
	...
	describe("with a valid access token", () => {
		...
		it("returns an error with an invalid dpop htu value (query params)", async () => {
			...
			const claims = {
				jti: "jti",
				htm: "POST",
				htu: "http://localhost:5000/deferred-credential",
				iat: Math.floor(Date.now() / 1000),
				ath: "ath",
			};
			const dpop = await new SignJWT(claims)
			...
			const response = await request(app)
				.post("/deferred-credential?test=true")
				.set("Authorization", `DPoP ${access_token}`)
				.set("DPoP", dpop)
				.send({ transaction_id });
		...
		describe("with a valid dpop header", () => {
			...
			it.skip("returns credentials with DPoP token type", async () => {
			...
			it.skip("returns credentials with bearer token type", async () => {
				const transaction_id = "full";
				const response = await request(app)
					.post("/deferred-credential")
