...
export function oauthAuthorizationServerHandlerFactory(config: OauthAuthorizationServerHandlerConfig) {
	return async function oauthAuthorizationServerHandler(
		expressRequest: Request,
	): Promise<OauthAuthorizationServerResponse> {
    const authorization_endpoint = new URL('/authorize', config.issuer_url).toString();
    const authorization_challenge_endpoint = new URL('/authorization-challenge', config.issuer_url).toString();
